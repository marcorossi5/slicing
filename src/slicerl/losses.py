...
def get_loss(setup):
    ...
    if setup.get('loss') == 'focal':
        ...
        from_logits = setup.get("from_logits")
        name        = "xent-l1"
        return CombinedLoss(from_logits=from_logits, name=name)
    elif setup.get('loss') == 'focal_l1':
        from_logits = setup.get("from_logits")
        name        = "focal_xent-l1"
        gamma       = setup.get("gamma")
        return CombinedFocalLoss(from_logits=from_logits, gamma=gamma, name=name)
    else:
