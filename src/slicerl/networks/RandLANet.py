...
class RandLANet(AbstractNet):
        ...
        if self.fc_type == "conv":
            self.fcs = [
                Conv1D(
                    ...
                    16,
                    padding="same",
                    input_shape=(None, None, None, iunits),
            ...
            raise NotImplementedError(
                f"First and final layers must be of type 'conv'|'dense', not {self.fc_type}"
            )

        if self.dropout_perc:
            self.dropout = [
                Dropout(self.dropout_perc, name=f"dropout{i+1}")
    ...
    def call(self, inputs):
        ...
        for i, (us, interp, ups, res) in enumerate(
            zip(self.USs, n_idxs[::-1], rndss[::-1], residuals[-2::-1])
        ):
